...
export default async function PreviewPage({
  ...
  return (
    <div>
      <div className="sticky top-14 z-30 flex items-center justify-between gap-3 border-b border-line bg-lagoon-900 px-4 py-2.5 text-white sm:px-6 lg:top-0">
        <Link
          href={`/app/itineraries/${trip.id}`}
          className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-medium text-lagoon-100 transition-colors hover:bg-white/10 hover:text-white"
        >
          <ArrowLeft className="h-4 w-4" />
          Back to editor
        </Link>
        <p className="hidden text-xs text-lagoon-200 sm:block">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className="inline-flex h-8 items-center gap-1.5 rounded-full bg-white px-3.5 text-xs font-semibold text-lagoon-900 transition-transform hover:-translate-y-px"
          >
